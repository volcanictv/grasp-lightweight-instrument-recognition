...
`model` mode (Milestone 3) measures params, FLOPs/MACs, model file size,
Titan Xp GPU latency (median/p95 over warm runs) and peak VRAM, batch
throughput, and ONNX Runtime CPU latency — the number that's portable to
hardware we don't own. Titan Xp numbers are never a deployment claim on
their own (CLAUDE.md).
